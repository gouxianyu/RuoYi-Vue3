...
<script setup name="BindRobot">
import { listRobot } from "@/api/control/robot";
import { bindRobot, getRobotsBySceneId } from "@/api/control/scene";
...
function getBindList() {
    getRobotsBySceneId(sceneId.value).then(res => {
...
function handleSelectRobot() {
    ...
    bindRobot({ sceneId: sceneId.value, robotIds: robotIds.value }).then(res => {
